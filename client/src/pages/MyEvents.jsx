...
import {
  ...
  TrendingUp, Moon, Sun, Bell, LayoutDashboard, PlusSquare,
} from "lucide-react";
  ...
  return (
    ...
    >
      ...
      <main className="flex-1 min-w-0 px-12 py-10 overflow-y-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-12">
          {STATS.map((stat) => (
            <div
              key={stat.id}
              className={`
                p-8 rounded-[40px] shadow-3xl border transition-all hover:-translate-y-2
                ${stat.id === 1
                  ? "bg-gradient-to-br from-indigo-600 to-blue-500 border-transparent"
                  : darkMode
                    ? "bg-[#1E0B3B] border-white/5"
            ...
            >
              ...
              <p
                className={`
                  ...
                  ${stat.id === 1 ? "text-indigo-100" : "text-slate-400"}
                `}
              >
                {stat.label}
              </p>
              <h3
                className={`
                  text-4xl font-black
                  ${stat.id === 1 ? "text-white" : darkMode ? "text-white" : "text-slate-900"}
                `}
              >
                {stat.value}
              </h3>
        ...
        <div
          className={`
            inline-flex p-2 rounded-[30px] mb-12
            ${darkMode ? "bg-[#1E0B3B]" : "bg-white shadow-sm border border-slate-100"}
          `}
        >
          <button
            onClick={() => setActiveTab("hosting")}
            className={`
              px-10 py-4 rounded-[24px] font-black text-sm transition-all
              ${activeTab === "hosting"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 pb-20">
          {HOSTED_EVENTS.map((event) => (
            <div
              key={event.id}
              className={`
                group rounded-[48px] overflow-hidden
                transition-all duration-500 border
                ${darkMode
                  ? "bg-[#1E0B3B] border-white/5 hover:border-indigo-500/30"
                  : "bg-white border-slate-100 shadow-sm hover:shadow-2xl hover:shadow-slate-200"
                }
              `}
            >
              {/* Image */}
              <div className="relative h-64">
                <img
                  ...
                  alt={event.title}
                />
                <div className="absolute top-5 left-5 px-4 py-2 rounded-xl bg-black/40 backdrop-blur-md text-white text-[10px] font-black uppercase tracking-widest">
                  {event.category}
                </div>
                <div className="absolute top-5 right-5 px-4 py-2 rounded-xl bg-indigo-600 text-white text-xs font-black shadow-lg">
                  {event.price}
                </div>
                <div className="absolute bottom-5 left-5 flex items-center gap-2 text-white font-black text-xs">
              ...
              {/* Info */}
              <div className="p-10">
                <h4 className="text-xl font-black mb-4 tracking-tight leading-tight group-hover:text-indigo-500 transition-colors">
                  {event.title}
                </h4>
                ...
                <div className="flex items-center gap-4 mb-8">
                  <div className="w-8 h-8 rounded-full bg-slate-200 border-2 border-white overflow-hidden flex items-center justify-center text-slate-500">
                    <Users size={16} />
                  </div>
                ...
                <div className={`pt-8 border-t flex items-center justify-between ${darkMode ? "border-white/5" : "border-slate-50"}`}>
                  <div>
                    <div className="flex items-center gap-2 text-[10px] font-black text-emerald-500 uppercase tracking-widest">
                      <TrendingUp size={12} /> {event.growth} growth
                    </div>
                  ...
                  <div className="flex items-center gap-2">
                    <div
                      className={`
                        w-8 h-8 rounded-lg flex items-center justify-center font-black text-xs text-white
                        ${event.id === 1 ? "bg-indigo-500" : event.id === 2 ? "bg-blue-500" : "bg-purple-500"}
                      `}
                    >
